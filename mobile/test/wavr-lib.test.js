...
test("parseCoreService keeps role=desktop -- a Core on a computer is still a Core", () => {
  // The role says which kind of machine hosts the Core, not whether it is one.
  // Rejecting desktop meant the phone discarded the advertisement of the Core
  // almost everybody installs, and reported that it had found nothing.
  const r = { name: "Wavr", ipv4Addresses: ["192.168.1.226"], port: 8000,
              txtRecord: { v: "1", role: "desktop", pv: "1" } };
  assert.deepEqual(parseCoreService(r), { name: "Wavr", host: "192.168.1.226", port: 8000 });
});
