...
(function(){
  ...
  // ---------- the dashboard's own translator, when it is there ----------
  // The shim draws two kinds of screen: overlays it owns before a hub exists,
  // and blocks it mounts INSIDE the dashboard. The second kind sits surrounded
  // by translated text — a Portuguese screen with an English paragraph in the
  // middle of it, permanently, on the card that explains what this device
  // sends about you.
  //
  // `window.WavrT` is published by `i18n.js`, the third script index.html
  // loads, so on those screens it is simply available and was never called.
  // Guarded anyway: an overlay drawn before the dashboard exists gets the
  // English literal rather than an exception, which is the right failure.
  function T(s, args){
    try{
      if(typeof window.WavrT === "function") return window.WavrT(s, args);
    }catch(_){}
    return s;
  }

  // ---------- "this is the same phone as last time" ----------
  // Pairing the same phone again used to leave the previous credential live.
  // Four re-pairs in one afternoon left the first user's hub holding four
  // working keys to his home, three of them forgotten and none of them
  // identifiable. The hub can now retire the older ones -- but only if the
  // phone tells it which phone it is, and nothing in a WebView survives a
  // reinstall, so the value has to come from native.
  //
  // Resolves to a hex digest, or null when the plugin is absent or cannot
  // produce one. Null is a normal answer: pairing then behaves exactly as it
  // did before (mint, retire nothing). Cached because it never changes within
  // a run, and both pairing paths ask for it.
  var _deviceKey;                                   // undefined = not asked yet
  function chaveDoAparelho(){
    if(_deviceKey !== undefined) return Promise.resolve(_deviceKey);
    if(!WavrNet || typeof WavrNet.deviceKey !== "function"){
      _deviceKey = null; return Promise.resolve(null);
    }
    return Promise.resolve(WavrNet.deviceKey()).then(function(r){
      var k = r && r.known && typeof r.key === "string" ? r.key.trim() : "";
      _deviceKey = k || null;
      return _deviceKey;
    }, function(){
      // Never fail a pairing over this. A duplicate credential is a mess; a
      // phone that cannot pair at all is a broken product.
      _deviceKey = null; return null;
    });
  }
  ...
  // Both go through `sensorAvailable()` so the build flag is ONE verdict rather
  // than a flag some readers honour and others do not. Without this, a phone
  // whose stored choice is sensor-only routes to the node screen -- disabled
  // Start, no dashboard, and no control anywhere to change the choice, because
  // the overlay that owns "Unpair this device" is only built on the dashboard
  // path. On a build that cannot sense, the honest fallback is the dashboard.
  function capsHasSensor(){ return sensorAvailable() && !!(_caps && _caps.sensor); }
  function capsSensorOnly(){ return capsHasSensor() && !_caps.viewer && !_caps.admin; }
  ...
  window.WAVR_MOBILE = {
    ...
    // "which phone is this" — resolves to a hex digest or null. Published
    // because the TYPED-CODE pairing form lives in the dashboard, not here,
    // and the dashboard has no way to reach a native plugin. Without this the
    // one path a person reaches by re-trying is the one that keeps minting a
    // second live key to their home.
    deviceKey: chaveDoAparelho,
  ...
  function showChooseCore(){
    ...
    noHubTimer = setTimeout(function(){
      ...
      try{ spin.remove(); }catch(_){}
      sub.textContent = "";
      // What this used to say: "Make sure your hub is on and this phone is on
      // the same network." Both were true when the first user read it. His Core
      // was up, advertising `_wavr._tcp` on 192.168.1.226, and his phone was on
      ...
      // check the network sends you to look at the half that is working.
      //
      // The VPN line only appears when a VPN is actually up. `networkFacts`
      ...
      if(net && typeof net.networkFacts === "function"){
  ...
  // Ask the SYSTEM to look, alongside the in-process browse.
  //
  // `Zeroconf.watch` runs JmDNS inside this app, and an app with a VPN cannot
  // send a multicast query to its own Wi-Fi: the tunnel carries `224.0.0.0/3`
  // and Android binds the app to the VPN by UID with `bypassable=false`, so
  // even binding the socket to the Wi-Fi address changes nothing. Measured on
  // a current flagship phone with a mainstream VPN — an ordinary setup, not an
  // exotic one. A `_wavrtest._tcp` service published from the laptop was
  // invisible to the phone at the same moment the laptop saw it instantly.
  //
  // `WavrNet.discoverCores` asks Android's own resolver instead, naming the
  // Wi-Fi network explicitly. That resolver lives outside this app's network
  // scope — it is the same one still advertising this phone on the Wi-Fi while
  // the app can see nothing.
  //
  // Both paths run from the start rather than one being a fallback: whichever
  // answers first wins, and neither has to fail before the other is tried. On a
  // phone with no VPN they simply agree.
  function startSystemDiscovery(onFound){
    var net = plugin("WavrNet");
    if(!net || typeof net.discoverCores !== "function") return false;
    var ver = function(){
      if(!_coreWatchActive) return;
      Promise.resolve(net.discoverCores({ type: "_wavr._tcp.", timeoutMs: 6000 }))
        .then(function(r){
          if(!_coreWatchActive) return;
          var lista = (r && r.services) || [];
          for(var i = 0; i < lista.length; i++){
            var s = lista[i];
            if(!s || !s.host || !s.port) continue;
            // Shaped like a ZeroConf `service` so `onFound` cannot tell the two
            // paths apart -- one consumer, one contract.
            onFound({ name: s.name || "Wavr", port: s.port,
                      ipv4Addresses: [s.host], hostname: s.host,
                      txtRecord: s.txt || {} });
          }
          if(_coreWatchActive) setTimeout(ver, 1200);   // keep looking while the screen is up
        })
        .catch(function(){ /* the in-process browse is still running */ });
    };
    ver();
    return true;
  }
  ...
  function startCoreWatch(onFound, onFailure){
    ...
    var sistema = startSystemDiscovery(onFound);
    function fail(){                       // report a native failure at most once
      if(!_coreWatchActive) return;
      // With the system path running, an in-process failure is not the end of
      // the search -- and `onFailure` is terminal: it settles the screen on
      // "Couldn't search for hubs on this network" and stops everything. The
      // one that still works has to be allowed to finish; the no-hub timer is
      // already there to close the screen honestly if neither finds anything.
      if(sistema) return;
  ...
  function startCameraScan(name){
    ...
    // other app is using it" — sending somebody to close other apps and try
    // again, forever, over a camera that had never been asked for anything. The
    // first user hit exactly that, after a build stopped shipping `jsqr.js`.
    //
    // `_marcaLeitor` is set only while the decoder is loading, so the handler
    // below can tell which half of the chain threw.
    var carregandoLeitor = true;
    _ensureJsQR().then(function(fn){
      jsQRfn = fn;
      carregandoLeitor = false;
  ...
  function onScannedPayload(text, name){
    ...
    WavrNet.probe({ url: _base }).then(function(r){
      ...
      persistPairing(_base, probed, null).then(function(){
        return chaveDoAparelho().then(function(chave){
          var corpo = { code: code, device_name: name };
          if(chave) corpo.device_key = chave;      // absent, never null: an older hub ignores it either way
  ...
  function showRequestPairing(){
    ...
    askBtn.onclick = function(){
      ...
      chaveDoAparelho().then(function(chave){
        var corpo = { requester_name: name, platform: "Android" };
        if(chave) corpo.device_key = chave;        // absent, never null: an older hub ignores it either way
        return netFetch(_base + "/api/pair-request", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify(corpo)
        });
      }).then(function(r){
  ...
  // Turned OFF for the first-user release candidate, deliberately, and this is
  // the ONE place that decides it -- the header pill, the node screen's Start
  // button and the onboarding path all read this, so they disappear together
  // rather than one of them surviving as a control that fails.
  //
  // WHY: the native service streams to `POST /api/telemetry`, and the Core no
  // longer has that route. The surviving one is `/api/nodes/telemetry`, which
  // takes a NODE token from the enrolment flow -- not the companion's pairing
  // token. It is a different authentication model, not a renamed path, so
  // there is no honest one-line fix.
  //
  // Shipping the button anyway would put a control in front of somebody that
  // is known to fail, and then ask them to ignore it. Set this back to the
  // capability check when the phone can enrol as a node.
  var SENSOR_MODE_IN_THIS_BUILD = false;
  function sensorAvailable(){
    if(!SENSOR_MODE_IN_THIS_BUILD) return false;
    return !!(WavrSensor && typeof WavrSensor.start === "function");
  }
  ...
  function showChooser(){
    ...
    // that cost the first user a round trip.
    //
    // "This is the core Wavr node" was the worse one. In this product **Core**
    // is the proper noun for the machine doing the thinking and **Node** is a
    // small sensor board -- both are admin words that `docs/VOCABULARY.md`
    // says a household is never shown. Used together here, in lower case, to
    // mean "this is the fundamental one", the sentence reads as "tick this to
    // turn your phone into the Core", which is the opposite of what the option
    // does. He read it and asked whether it was what he wanted, which is the
    // question the sentence exists to answer.
    //
    // The admin line said only "Needs an admin code from the hub" and left out
    // the single fact that decides whether it works: the code has to be minted
    ...
    // button and no way back; and it is worse than a shorter screen, because
  ...
  function injectSensorPill(){
    // The node screen already refuses when the sensor path is unavailable; this
    // pill did not, so it appeared in the header regardless and was the FIRST
    // thing a paired phone offered. Same gate, same moment.
    if(!sensorAvailable()) return;
    ensureSensor();
  ...
  function showDetails(){
    ...
    card.appendChild(el("span", "wavrm-lab", T("This device — what it sends")));
    var sendRow = el("div", "wavrm-inrow");
    sendRow.appendChild(el("span", "wavrm-inrow-l", T("Tap to reduce · hold to turn off")));
  ...
  function injectConsentTile(){
    ...
    box.appendChild(el("span", "wavrm-ct-lab", T("This device — what it sends")));
    box.appendChild(el("p", "wavrm-sub",
      T("The level above is how much of your home Wavr senses. This is what THIS device sends about you.")));
    var row = el("div", "wavrm-inrow");
    row.appendChild(el("span", "wavrm-inrow-l", T("Tap to reduce · hold to turn off")));
    row.appendChild(makeConsentControl().btn);
    box.appendChild(row);
