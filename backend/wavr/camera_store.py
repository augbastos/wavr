...
class CameraStore:
    """Persisted camera DEFINITIONS (name/room/rtsp_url/confidence/mac). Never stores an
    ...
    def _migrate(self) -> None:
        # Migration-safe additive column (mirrors device_meta._migrate): a DB created
        # before the F3 `mac` column existed lacks it; CREATE TABLE IF NOT EXISTS won't
        # add it, so ALTER it in when absent.
    ...
    def add(self, name: str, room: str, rtsp_url: str, confidence: float,
        ...
        self._conn.execute(
            "INSERT INTO cameras (name, room, rtsp_url, confidence, mac)"
            " VALUES (?, ?, ?, ?, ?)",
            (name, room, rtsp_url, confidence, mac),
        )
        self._conn.commit()
    ...
    def list(self) -> list[dict]:
        rows = self._conn.execute(
            "SELECT name, room, rtsp_url, confidence, mac FROM cameras ORDER BY name"
        ).fetchall()
        return [dict(r) for r in rows]

    def get(self, name: str) -> dict | None:
        r = self._conn.execute(
            "SELECT name, room, rtsp_url, confidence, mac FROM cameras WHERE name = ?",
            (name,),
        ).fetchone()
