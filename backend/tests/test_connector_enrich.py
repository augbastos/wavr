...
def test_open_meteo_enabled_sends_minimal_egress(monkeypatch):
    monkeypatch.setenv("WAVR_HOME_LAT", "40.69")
    monkeypatch.setenv("WAVR_HOME_LON", "-74.05")
    store = _enabled_store("open-meteo")
    ...
    assert "latitude=40.69" in url and "longitude=-74.05" in url
...
def test_open_meteo_coarsens_lat_lon_before_egress(monkeypatch):
    ...
    monkeypatch.setenv("WAVR_HOME_LAT", "40.689247")
    monkeypatch.setenv("WAVR_HOME_LON", "-74.044502")
    store = _enabled_store("open-meteo")
    spy = _Spy(result={"current": {"temperature_2m": 14.2, "precipitation": 0.0,
                                    "weather_code": 3, "wind_speed_10m": 12.1}})
    fetch = open_meteo.make_weather_fetch(store, get=spy)
    ...
    (url,), _kwargs = spy.calls[0]
    assert "latitude=40.69" in url and "longitude=-74.04" in url
    assert "40.689247" not in url and "74.044502" not in url  # full precision never leaves
