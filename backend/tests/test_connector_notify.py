...
def test_sanitize_summary_redacts_coordinate_like_pair():
    out = telegram._sanitize_summary("last seen near 40.6892, -74.0445")
    assert "40.6892" not in out and "-74.0445" not in out
    assert "[redacted]" in out
...
def test_build_alert_text_redacts_mac_and_coordinate_and_rtsp_and_frame():
    text = telegram.build_alert_text(
        ...
        "MAC AA:BB:CC:DD:EE:FF at 40.6892, -74.0445 via rtsp://cam.local/frame_1.jpg")
    assert "AA:BB:CC:DD:EE:FF" not in text
    assert "40.6892" not in text and "-74.0445" not in text
...
def test_send_redacts_summary_before_it_reaches_the_wire(monkeypatch):
    ...
    result = send("rogue_device", "alert", "kitchen",
                   "MAC AA:BB:CC:DD:EE:FF near 40.6892, -74.0445 rtsp://cam.local/frame_9.jpg")
