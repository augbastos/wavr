...
def test_brief_miss_does_not_fire_a_false_departure():
    ...
    t.update({"Alex"})            # baseline
    t.update(set())                  # miss #1
    t.update({"Alex"})            # back before grace -> streak resets, no left edge
...
def test_round_trip_leave_then_return():
    ...
    t.update({"Alex"})            # baseline home
    t.update(set())                  # grace 1 -> left
    t.update({"Alex"})            # arrived again
    assert edges == [("Alex", False), ("Alex", True)]
...
def test_only_persons_the_caller_feeds_ever_edge():
    ...
    t, edges = _tracker()
    t.update({"Alex"})            # baseline
    t.update({"Alex"})            # an anonymous device present but NOT named -> not fed
