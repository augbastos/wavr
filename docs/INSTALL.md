...
were built and installed **silently** on one Windows machine with every Python
...
**Exercised on real distros in CI, never on Pi hardware** — see the verification table at the top. What is genuinely untested is the Raspberry Pi itself (see the
...
- **The image has to contain `frontend/`, and now does.**
  ...
  layout, not by an actual `docker compose up` — Docker was not available where this was written.
