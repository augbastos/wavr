# Wavr — Documento de Design (Revisão 3: fusão multi-modal + control plane)
...
```json
{
  ...
  "sources": [
    {"modality": "wifi_csi", "presence": true,  "confidence": 0.9},
    {"modality": "camera",   "presence": false, "confidence": 0.0}
  ],
  "explanation": "wifi_csi: presente · camera: vazio (off) → 76% ocupado",
  "ts": "2026-07-01T16:20:01+00:00"
}
```

`confidence = agreement × strength` — `agreement` = fração da massa confiável que diz "presente",
`strength` = melhor evidência presente (peso × confiança da fonte). Assim uma fonte fraca sozinha
nunca chega a 100%. **`network` é sinal nível-casa** (aparece como o pseudo-cômodo "casa"), não
por-cômodo — no v1 não corrobora cômodos específicos; usá-lo como prior é refinamento futuro.
...
- Estado em memória — **câmeras sempre bootam OFF** (sem estado ON persistido; é o mais seguro);
  indicador ON/OFF **visível** no dashboard; controle pela API genérica de fontes
  (`GET /api/system`, `POST /api/sources/{nome}/toggle`), protegida pelo header `X-Wavr-Local`.
...
- [ ] Toggle de segurança: desligar a câmera fecha o RTSP e zera eventos daquela câmera; câmeras
      começam desligadas no boot; estado visível (sempre boota OFF, sem ON persistido).
...
- [ ] Ligar/desligar fontes em runtime (API `/api/sources/{nome}/toggle`) + registrar fontes no
      código (lista `sources` do `create_app`), sem tocar dashboard/fusão.
